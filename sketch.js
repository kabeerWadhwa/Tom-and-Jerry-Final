...
var tom, Jerry,Cheese,JerryGroup,score
...
function preload(){
  ...
  happyTom = loadImage("Images/Tom catching Jerry.jpeg")
}
function setup() {
...
function runningJerry(){
  if(World.frameCount%100===0){
    var Jerry = createSprite(600,400,20,20)
    Jerry.addImage(JerryIMG)
    Jerry.scale = 0.2
    Jerry.velocityX = Math.round(random(-1,7))
    Jerry.velocityY = Math.round(random(-1,7))
    Jerry.y = Math.round(random(200,1000))
    Jerry.x = Math.round(random(300,600))
...
function draw() {
  ...
  if(JerryGroup.isTouching(tom)){
    textSize(20)
    fill("white")
    JerryGroup.destroyEach();
    tom.changeImage(happyTom)
    score = score + 10
  }
  textSize(15)
  fill("white")
